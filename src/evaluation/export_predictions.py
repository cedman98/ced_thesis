...
each fold's held-out predictions, reusing each trainer's exact params (imported,
not copied, so they can't drift). Only the cheap tree/glass-box models are
...
def export(matrix_path="data/processed/ml_training_matrix.parquet",
    ...
    # Adjacent purged folds share one inclusive boundary hour; drop those dupes so
    # each (model, tech, timestamp) is held out exactly once.
    res = res.drop_duplicates(subset=["technology", "model", "timestamp"], keep="last")
